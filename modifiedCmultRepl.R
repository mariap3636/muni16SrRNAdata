modifiedCmultRepl<-function (X, label = 0, method = c("GBM", "SQ", "BL", "CZM", 
    "user"), output = c("prop", "counts"), delta = 0.65, threshold = 0.5, 
    correct = TRUE, t = NULL, s = NULL, suppress.print = FALSE) 
{
    ...
    if (method != "CZM") {
        ...
        s <- switch(method, GBM = 1/apply(t, 1, function(x) exp(mean(log(x+0.0001 )))), 
